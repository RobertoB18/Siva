...
export default function () {
    ...
    if(params.idVenta){
      ...
      fetch(`/api/sales/${params.idVenta}`)
        ...
        .then(async data => {
          ...
          setUseCFDI(data.use)
          setPaymentForm(data.metodoPago)
  ...
  const finishedSale = async () => {
    ...
    const payload = {
      clienteId: clientSelected ? Number(clientSelected.id) : null,
      use: useCFDI ? useCFDI : null,
      pago: paymentForm ? paymentForm : null,
      storeId: selectedStore,
      sale: Number(params.idVenta),
    }
        ...
        <table className="table-auto border-collapse border border-gray-300 w-full">
          <thead>
            <tr className="bg-black text-white">
              ...
              {!params.idVenta && <th className="border px-4 py-2">Acciones</th>}
            </tr>
          </thead>
          <tbody>
            {cart.map((item) => (
              <tr key={item.id}>
                <td className="border px-4 py-2 text-center">
                  <input
                    ...
                    value={item.quantity}
                    disabled={params.idVenta}
                    className={`${params.idVenta ? "cursor-none" : "cursor-pointer"} w-auto text-center`}
        ...
        {valido && (
          <>
            <button
              ...
              onClick={() => (!params.idVenta ? setFactura(true) : finishedSale())}
            >
              {params.idVenta ? "Actualizar Venta" : "Finalizar venta"}
            </button>
            <button
